...
- (id)initWithImageFile:(NSString*)filename
{
    ...
    if (nil != self) {
        if (NO == [self loadImageFromFile:filename]) {
...
- (id)initWithUIImage:(UIImage * )image
{
    ...
    if (nil != self) {
        if (NO == [self loadImageFromUIImage:image]) {
            NSLog(@"Failed to load texture image from Image ");
            self = nil;
        }
    }
...
- (BOOL)loadImageFromFile:(NSString*)filename
{
    BOOL ret = NO;
...
- (BOOL)loadImageFromUIImage :(UIImage * )image
{
    BOOL ret = NO;
    ...
    if (image) {
        ...
        //CGImageRef cgImage = CGImageCreateWithImageInRect(uiImage.CGImage,CGRectMake(0,0,512,512));
        // Get the image size
        _width = (int)CGImageGetWidth(cgImage);
        _height = (int)CGImageGetHeight(cgImage);

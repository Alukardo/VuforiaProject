...
@implementation MyAppMenuViewController

@synthesize showingMenu;
@synthesize windowTapGestureRecognizerAdded;
@synthesize windowTapGestureRecognizer;
...
- (void)viewDidLoad {
    ...
    //------Calaculate Frame Size Part---------
    // When the view appears, we adjust the frame size
    CGFloat menuWidthScale = [MyAppMenuViewController getMenuWidthScale];
    ...
    windowTapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapGestureAction:)];
    ...
    //------Get Singleton MaskImage---------
    if([MaskImage GetSingletonMaskImage].maskImage){
        self.maskImage.image = [MaskImage GetSingletonMaskImage].maskImage;
    }else{
        NSLog(@"# ALK:Maskimg in viewload is nil!");
    }
    if([MaskImage GetSingletonMaskImage].scalImage){
        self.maskImage.image = [MaskImage GetSingletonMaskImage].scalImage;
    }else{
        NSLog(@"# ALK:Scalimg in viewload is nil!");
    }
    [[MaskImage GetSingletonMaskImage] maskRest];
}
...
- (IBAction)CutMaskImage:(UIButton *)sender {
    ...
    MaskImage * mimg = MaskImage.GetSingletonMaskImage;
    if(mimg.origImage && mimg.scalImage){
        //mimg.maskImage = mimg.scalImage;
        mimg.maskImage = [mimg.scalImage GrabFrontImage:mimg.scalImage];
        NSString *imagePath = [NSHomeDirectory() stringByAppendingString:@"/Documents/mask.png"];
        [UIImagePNGRepresentation(mimg.maskImage) writeToFile:imagePath atomically:YES];
        
        [mimg setFinished:true];
        //[mimg printMASK];
    }
    self.maskImage.image = mimg.maskImage;
    self.scaleImage.image = mimg.scalImage;
    [self loadImageFinished:self.maskImage.image];
    //hhaahh
    NSLog(@"GrabCut with Image size ( %d , %d )",(int)self.maskImage.bounds.size.width,(int)self.maskImage.bounds.size.height);
}
- (IBAction)maskType:(UIButton *)sender
{
    
    if([sender.titleLabel.text  isEqualToString: @"BGD"]){
        [sender setTitle:@"FGD" forState:UIControlStateNormal];
    }
    else if([sender.titleLabel.text  isEqualToString: @"FGD"]){
        [sender setTitle:@"PR_BGD" forState:UIControlStateNormal];
        
    }
    else if([sender.titleLabel.text  isEqualToString: @"PR_BGD"]){
        [sender setTitle:@"PR_FGD" forState:UIControlStateNormal];
        
    }
    else if([sender.titleLabel.text  isEqualToString: @"PR_FGD"]){
        [sender setTitle:@"BGD" forState:UIControlStateNormal];
        
    }
    [[MaskImage GetSingletonMaskImage] changMaskPointType];
...
- (void)tapGestureAction:(UITapGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateEnded)
    {
        ...
        if (![self.view pointInside:[self.view convertPoint:location fromView:self.view.window] withEvent:nil])
        {
            ...
            NSLog(@"ALK  outside View");
            [self dismissMenu];
        }
        CGPoint taplocation = [sender locationInView:self.maskImage];
        if (taplocation.x > 0 && taplocation.x < 255 && taplocation.y > 0 && taplocation.y < 255) {
            [[MaskImage GetSingletonMaskImage] changMaskWithX:taplocation.x andY:taplocation.y];
            NSLog(@" # ALK tap mask point (%d , %d)",(int)taplocation.x,(int)taplocation.y);
        }
    }
}
- (void)panGestureAction:(UIPanGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateChanged) {
        ...
        CGPoint currentPoint = [sender locationInView:self.maskImage];
        int x = (int)currentPoint.x;
        int y = (int)currentPoint.y;
        
        if( x > 0 && x < 255 && y > 0 && y < 255){
            
            [[MaskImage GetSingletonMaskImage] changMaskWithX:x andY:y];
            NSLog(@" # ALK pan mask point (%d , %d)",x,y);
        }
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([itemName isEqualToString:self.dismissItemName]) {
        ...
        if (sw != nil) {
            // CASE 1: the selected cell contains a UISwitch
            ...
            // CASE 2: the selected cell is a "radio button" cell in a "radio group"
            ...
            // CASE 3:
            // the selected cell is neither a "Switch" nor a "radio button"(it's just a labeled item)
            // we notify the menu delegate about the selected item

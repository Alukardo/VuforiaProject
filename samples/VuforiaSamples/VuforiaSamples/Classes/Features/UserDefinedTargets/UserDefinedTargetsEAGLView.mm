...
namespace {
    ...
    // Model scale factor
    const float kObjectScale = 75.0f;
}
...
//        }
        //ALK TODO (GLvoid*) (GLvoid*)[augmentationTexture[i] pngData]
        glGenTextures(1, &textureID);
...
- (void)renderFrameWithState:(const Vuforia::State &)state projectMatrix:(Vuforia::Matrix44F &)projectionMatrix
{
    ...
    //TODO
    MaskImage * maskimageForEGALview = [MaskImage GetSingletonMaskImage];
    if([maskimageForEGALview getFinished]){
        augmentationTexture[0] = [[Texture alloc] initWithUIImage:maskimageForEGALview.maskImage];
        [maskimageForEGALview setFinished:false];
    }
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        SampleApplicationUtils::rotatePoseMatrix(90, 0, 0, 1,&modelViewMatrix.data[0]);
        ...
        glVertexAttribPointer(vertexHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)MypanleVertices);
        glVertexAttribPointer(normalHandle, 3, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)MypanleNormals);
        glVertexAttribPointer(textureCoordHandle, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid*)MypanleTexCoords);
        ...
        glDrawElements(GL_TRIANGLES, kNumMypanleIndices, GL_UNSIGNED_SHORT, (const GLvoid*)MypanleIndices);
